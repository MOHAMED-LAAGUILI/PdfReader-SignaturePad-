...
import { generateSignedPdf, exportToJson, downloadBlob } from './utils/pdfUtils';
import ErrorAlert from './components/ErrorAlert';
import PdfViewer from './components/PdfViewer/PdfViewer';
import { SignatureTool, StampTool, TextTool, ActionsTool } from './components/ToolsPanel';
...
function App() {
  ...
  const handleExportJson = () => {
    ...
    try {
      const jsonStr = exportToJson(fileName, pdfUrl, signatures, stamps, textAnnotations);
  ...
  const handleSaveToServer = async () => {
    ...
    try {
      setIsLoading(true);
      const jsonData = exportToJson(fileName, pdfUrl, signatures, stamps, textAnnotations);
        ...
        >
          {text.content}
              ...
              <div className="w-full lg:w-80 bg-gray-50 p-4 rounded-md">
                ...
                <SignatureTool 
                  onAddSignature={(signatureData) => addSignature(signatureData, pageNumber)} 
                />
                
                <StampTool 
                  onAddStamp={(text, color) => addStamp(text, color, pageNumber)} 
                />
